...
void main(void){
		...
		vPos = aPos;
		v_eye = (uCameraMatrix * gl_Position).xyz;
}
...
//	float z = (2.0 * u_near) / (u_far + u_near - d * (u_far - u_near));
	float z = 2.0 * u_near * u_far / (u_far -u_near)/(d - (u_near +u_far)/(u_far-u_near));
	return z;
}
...
float[] bayer = float[](
 ...
 vec3 raytrace( vec3 pos, vec3 angle,float rough){
	...
	for(int i=0;i<maxstep;i++){
		//少しずつレイを進めてレイの深度がデプスマップより深くなったら終了
...
void main(void){ 
	...
	float z = texture(uDepthMap,vUv).r;
	...
	z = uintBitsToFloat(ueye.z);
//	z = z*0.5+0.5;
	z = -toLinear(z);
//	z = (z *(u_far -u_near) +u_near);
//	pos =( anglePos + v_eye * z);
	if(opacity < 1.0){
	...
	/*全反射*/ 
	//angle = normal;//reflect(eye,normal); 
	angle = reflect(eye,normal); 
	//angle = eye-2.0*dot(eye,normal) *normal;
	...
	//	vec2 ray2 = angle2uv(angle) * vec2(1.0,0.5); 
	//	result.rgb = textureTri(uEnvMap,vec2(256.0),ray2,0.0);
	//	result.rgb = angle;
	}
